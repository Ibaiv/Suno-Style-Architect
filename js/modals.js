...
function getModalsHTML() {
    ...
    <!-- STYLE SYNC MODAL (Stil-Synchronisator) -->
    <div id="style-sync-modal" class="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 hidden modal-enter-from">
        <div class="modal-content bg-[#0f111a] border border-purple-500/20 rounded-3xl shadow-2xl p-8 w-full max-w-2xl relative overflow-hidden">
            <!-- Background Glow -->
            ...
            <div class="absolute bottom-0 right-0 w-48 h-48 bg-indigo-500/10 rounded-full blur-3xl translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
            ...
            <div class="flex justify-between items-center mb-6 relative z-10">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-bold text-white tracking-tight">Stil-Synchronisator</h2>
                </div>
                <button class="close-modal-button text-neutral-500 hover:text-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            ...
            <div class="flex gap-2 mb-6 relative z-10">
                <button id="style-sync-tab-encode" class="style-sync-tab active flex-1 py-3 px-4 rounded-xl font-medium text-sm transition-all bg-purple-500/20 border border-purple-500/40 text-purple-300">
                    🎵 → 🖼️ Sound zu Bild
                </button>
                <button id="style-sync-tab-decode" class="style-sync-tab flex-1 py-3 px-4 rounded-xl font-medium text-sm transition-all bg-neutral-800/50 border border-neutral-700 text-neutral-400 hover:text-neutral-200">
                    🖼️ → 🎵 Bild zu Sound
                </button>
            </div>

            <!-- Tab Content: Encoding (Sound → Image) -->
            <div id="style-sync-encode-content" class="relative z-10">
                <p class="text-neutral-400 text-sm mb-4">Generiere ein visuelles Archetyp-Bild, das deinen aktuellen Musik-Prompt symbolisch darstellt.</p>
                
                <div id="style-sync-encode-preview" class="bg-neutral-900/50 border border-neutral-700 rounded-xl min-h-[200px] mb-4 flex items-center justify-center overflow-hidden">
                    <p class="text-neutral-500 text-sm" id="style-sync-encode-placeholder">Das Archetyp-Bild erscheint hier...</p>
                    <img id="style-sync-encode-image" class="hidden max-h-[300px] w-auto rounded-lg" alt="Generierter Archetyp"/>
                </div>
                
                <button id="style-sync-generate-archetype" class="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold py-3 px-4 rounded-xl shadow-lg shadow-purple-900/20 transition-all flex items-center justify-center gap-2">
                    <span id="style-sync-generate-text">Archetyp generieren</span>
                    <svg id="style-sync-generate-loader" class="animate-spin h-5 w-5 text-white hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                </button>
                <p id="style-sync-encode-error" class="text-red-400 text-xs mt-2 hidden"></p>
            </div>

            <!-- Tab Content: Decoding (Image → Sound) -->
            <div id="style-sync-decode-content" class="hidden relative z-10">
                <p class="text-neutral-400 text-sm mb-4">Lade ein Bild hoch und lass die KI einen passenden Musik-Prompt daraus ableiten.</p>
                
                <!-- Drop Zone -->
                <div id="style-sync-drop-zone" class="border-2 border-dashed border-neutral-700 rounded-xl min-h-[180px] mb-4 flex flex-col items-center justify-center cursor-pointer transition-all hover:border-purple-500/50 hover:bg-purple-500/5">
                    <div id="style-sync-drop-placeholder" class="text-center p-6">
                        <div class="w-12 h-12 bg-neutral-800 rounded-xl mx-auto mb-3 flex items-center justify-center">
                        ...
                        <p class="text-neutral-400 text-sm mb-1">Bild hierher ziehen</p>
                        <p class="text-neutral-500 text-xs">oder klicken zum Auswählen</p>
                    </div>
                    <img id="style-sync-decode-preview" class="hidden max-h-[200px] w-auto rounded-lg" alt="Hochgeladenes Bild"/>
                </div>
                <input type="file" id="style-sync-file-input" accept="image/*" class="hidden"/>
                
                <!-- Decoded Output -->
                <div id="style-sync-decode-output" class="hidden mb-4">
                    <label class="block text-xs font-medium text-neutral-400 uppercase tracking-wide mb-2">Generierter Musik-Prompt</label>
                    <div class="bg-neutral-900/70 border border-neutral-700 rounded-xl p-4">
                        <pre id="style-sync-decoded-prompt" class="text-sm text-neutral-200 whitespace-pre-wrap"></pre>
                    </div>
                </div>
                
                <div class="flex gap-3">
                    <button id="style-sync-analyze-image" class="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold py-3 px-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed" disabled>
                        <span id="style-sync-analyze-text">Bild analysieren</span>
                        <svg id="style-sync-analyze-loader" class="animate-spin h-5 w-5 text-white hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    </button>
                    <button id="style-sync-apply-prompt" class="hidden bg-green-600 hover:bg-green-500 text-white font-semibold py-3 px-6 rounded-xl shadow-lg transition-all">
                        In Vision übernehmen
                    </button>
                </div>
                <p id="style-sync-decode-error" class="text-red-400 text-xs mt-2 hidden"></p>
            </div>
        </div>
    </div>
